...
# Phase timings as fraction of total t in [0, 1]
# 0: show 2D data, 1: "no line" note, 2: formula, 3: lift, 4: surface,
# 5: hyperplane, 6: project circle, 7: rotate, 8: summary
breaks = [0, 0.10, 0.18, 0.28, 0.45, 0.58, 0.72, 0.85, 1.0]
labels = [
    "2D data on z=0 plane — no straight line can separate them.",
    ...
    "Project back to 2D: x^2 + y^2 = c",
    "Camera rotation — observe the separation",
    "In 3D: linear hyperplane  |  In 2D: nonlinear boundary",
]
...
def update(frame):
    ...
    elif t >= breaks[5] + 0.05 and t < breaks[7]:
        # Surface stays visible during hyperplane and rotate phases
        surf_alpha = 0.22
    elif t >= breaks[7]:
        surf_alpha = 0.22
    ...
    elif t >= breaks[6] and t < breaks[7]:
        plane_alpha = 0.35  # stays during projection
    elif t >= breaks[7]:
        plane_alpha = 0.35
...
ani.save("outputs/phase1_kernel_trick.gif", writer="pillow", fps=fps)
print("Phase 1 animation saved to outputs/phase1_kernel_trick.gif")
plt.close()
